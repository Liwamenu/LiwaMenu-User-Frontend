...
const updateReservationStatusSlice = createSlice({
  ...
  initialState,
  reducers: {
    resetUpdateReservationStatus: (state) => {
      state.loading = false;
      state.success = false;
      state.error = null;
      state.data = null;
...
export const updateReservationStatus = createAsyncThunk(
  "Reservations/OwnerUpdateStatus",
  async (data, { rejectWithValue }) => {
    try {
      const res = await api.put(
        `${baseURL}Reservations/OwnerUpdateStatus`,
        data,
      );
      return res.data;
    } catch (err) {
...
export const { resetUpdateReservationStatus } =
  updateReservationStatusSlice.actions;
